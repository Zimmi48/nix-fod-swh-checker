...
- `env` is normalized by `nix.py:_normalize_env`. Some derivation types (notably those produced by `fetchzip`, `fetchFromGitHub`, and similar helpers) serialize the entire derivation environment as a JSON string in the `__json` env variable. When present, this JSON object is parsed and merged into the env so that the fields below are read transparently.

`nix.py:_extract_origin_urls` collects upstream URLs from the derivation environment: the `url` variable (single URL) and the `urls` variable (whitespace-separated mirror URLs in the legacy format, or a JSON array when read from `__json`). These are stored on `FixedOutputDerivation.origin_urls` and copied into each `SWHCheckResult`.
