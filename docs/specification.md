...
The available subcommands are: `check`, `request` (or `request-archiving`), `generate` (or `generate-swh-fods`), `cook` (or `cook-swh-fods`), and `build` (or `build-swh-fods`).
...
| `--unpack-timeout` `<seconds>` | `30.0` | Timeout for unpacking each archive when checking its contents locally. |
...
#### `request` (or `request-archiving`) options
...
#### `request` (or `request-archiving`) exit codes
...
#### `generate` (or `generate-swh-fods`) options
...
#### `generate` (or `generate-swh-fods`) exit codes
...
When not `--quiet`, the command prints each SWHID along with the status of its cooking task (for example `new`, `pending`, `done`, or `failed`). This lets you see the current state of previously requested tasks and whether the `build` (or `build-swh-fods`) command may be run.

#### `cook` (or `cook-swh-fods`) options
...
#### `cook` (or `cook-swh-fods`) exit codes
...
#### `build` (or `build-swh-fods`) options
...
#### `build` (or `build-swh-fods`) exit codes
...
The checkpoint file is a JSON document used to resume interrupted `check` runs and to feed results into `generate` (or `generate-swh-fods`), `cook` (or `cook-swh-fods`), `build` (or `build-swh-fods`), and `request` (or `request-archiving`).
...
These URLs are stored in the `origin_urls` field of each result and in checkpoint files. They are used by `request` (or `request-archiving`) to ask Software Heritage to archive origins that are not yet in the archive.

Not every FOD has a usable origin URL. Some FODs are produced by complex build steps, and their environment contains no `url`/`urls` variable. `request` (or `request-archiving`) simply skips such FODs.
...
| `XDG_CACHE_HOME` | `check`, `generate` (or `generate-swh-fods`), `cook` (or `cook-swh-fods`), `build` (or `build-swh-fods`), `request` (or `request-archiving`) | Base directory for the default checkpoint file and the shared cache. |
